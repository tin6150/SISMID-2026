...
# 08_evaluation_xgb.R - XGBoost Forecast Evaluation using scoringutils
...
# Outputs
forecast_scores_path <- file.path(output_dir, "XGB_forecast_scores.csv")
summary_scores_path <- file.path(output_dir, "XGB_forecast_scores_by_horizon.csv")

# Strict validations: stop if target path lacks the XGB_ prefix
if (!grepl("XGB_", basename(forecast_scores_path))) {
  stop("Validation failed: per-forecast output path lacks the 'XGB_' prefix.")
}
if (!grepl("XGB_", basename(summary_scores_path))) {
  stop("Validation failed: summary output path lacks the 'XGB_' prefix.")
}
...
cat("\n====================================================================\n")
cat("Per-Reference-Date Forecast Scores Table - XGBoost (First 10 Rows):\n")
cat("====================================================================\n")
print(head(forecast_scores_table, 10))
cat("====================================================================\n\n")

cat("====================================================================\n")
cat("Forecast Evaluation Summary (scoringutils) by Horizon - XGBoost:\n")
cat("====================================================================\n")
...
message("XGBoost evaluation script completed successfully.")
