# 06_evaluation_wval.R
# Mirror of 04_evaluation.R for the wastewater-informed ARIMAX forecasts.
#
# Scores WVAL_flusight_forecasts.csv against observed truth, one score per
...
# output/data/04_evaluation/WVAL_forecast_scores.csv and
...
# output paths, and the `model` label differ. The baseline (unprefixed) score
# files are never written.
#
# See rules.md ("Wastewater Forecast Evaluation") and AGENTS.md.
...
scores_csv   <- "output/data/04_evaluation/WVAL_forecast_scores.csv"
summary_csv  <- "output/data/04_evaluation/WVAL_forecast_scores_by_horizon.csv"

# Refuse to write over 04_evaluation.R's baseline outputs.
if (!grepl("^WVAL_", basename(scores_csv)))  stop("Refusing to write: scores CSV lacks WVAL_ prefix")
if (!grepl("^WVAL_", basename(summary_csv))) stop("Refusing to write: summary CSV lacks WVAL_ prefix")
...
cat("\n--- WVAL forecast scores (per reference_date x horizon) ---\n")
...
cat("\n--- WVAL forecast scores summarized by horizon ---\n")
