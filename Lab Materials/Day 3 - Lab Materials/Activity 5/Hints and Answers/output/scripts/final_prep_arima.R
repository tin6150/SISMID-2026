# final_prep_arima.R
# Final FluSight submission prep (ARIMA) -- no modeling.
#
# Splits the finished ARIMA forecast file
# (output/data/03_forecast/flusight_forecasts.csv) into one CSV per
# reference date, written to output/data/final_flusight_submission_ARIMA/ as
# {YYYY-MM-DD}-Amanda-ARIMA.csv.
#
...
#
# Mirrors final_prep.R (the XGBoost version). See rules.md
# ("Final FluSight Submission Prep") and AGENTS.md.
...
submit_dir   <- "output/data/final_flusight_submission_ARIMA"

# Guard: only ever write into the ARIMA submission folder.
if (basename(submit_dir) != "final_flusight_submission_ARIMA") {
  stop("Refusing to write: output directory is not final_flusight_submission_ARIMA")
}
...
FNAME_PATTERN <- "^\\d{4}-\\d{2}-\\d{2}-Amanda-ARIMA\\.csv$"
...
for (rd in ref_dates) {
  ...
  fname <- paste0(rd_str, "-Amanda-ARIMA.csv")
...
if (!all(grepl(FNAME_PATTERN, written_files))) {
  stop("Some filenames do not match {YYYY-MM-DD}-Amanda-ARIMA.csv")
}
cat("[val] all filenames match {YYYY-MM-DD}-Amanda-ARIMA.csv: OK\n")
