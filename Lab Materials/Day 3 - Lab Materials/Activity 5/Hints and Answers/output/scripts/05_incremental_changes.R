# 05_incremental_changes.R
# Wastewater-informed variant of 03_forecast.R.
#
# Same pipeline as 03_forecast.R -- reads cleaned flu admissions, assigns
# seasons, fits an expanding-window model per reference date over the 2025-26
# testing season, and emits 1/2/3-week-ahead forecasts in FluSight long format
# (23 quantiles x 3 horizons per reference date) -- with ONE modeling change:
# auto.arima() is given national NWSS wastewater viral activity (WVAL),
# lagged 3 weeks, as an external regressor (ARIMAX).
#
# Why lag 3? It is the maximum forecast horizon, so every regressor value the
# three horizons need is already observed at the reference date r:
#
#   horizon 1 -> target r + 7  -> regressor week r - 14  (observed)
#   horizon 2 -> target r + 14 -> regressor week r - 7   (observed)
#   horizon 3 -> target r + 21 -> regressor week r       (observed)
#
# No future wastewater value is ever used, so the evaluation reflects a
# genuine real-time forecast. A shorter lag correlates more strongly in-sample
# but would require wastewater dated after r, leaking future information.
#
# All outputs carry a WVAL_ prefix; 03_forecast.R's outputs are never touched.
#
# See rules.md ("Forecasting with a Wastewater Leading Indicator") and AGENTS.md.
...
wastewater_csv <- "data/NWSSWVALNational.csv"
forecast_csv  <- "output/data/03_forecast/WVAL_flusight_forecasts.csv"
forecast_png  <- "output/figures/03_forecast/WVAL_forecast_vs_observed.png"

# Rule 7W: refuse to write over the baseline (unprefixed) artifacts.
if (!grepl("^WVAL_", basename(forecast_csv))) stop("Refusing to write: forecast CSV lacks WVAL_ prefix")
if (!grepl("^WVAL_", basename(forecast_png))) stop("Refusing to write: forecast PNG lacks WVAL_ prefix")
...
if (!file.exists(wastewater_csv)) stop(paste0("Missing input: ", wastewater_csv))

# ---- Rule 1: Input Data (flu admissions) -------------------------------------
...
# ---- Rule 0W: Second Input -- national wastewater (NWSS WVAL) ----------------
cat("\n--- Rule 0W validations (wastewater input) ---\n")

ww_raw <- read_csv(wastewater_csv, col_types = cols(.default = "c"))

ww_cols <- c("Pathogen Target", "Week End", "National WVAL")
if (!all(ww_cols %in% names(ww_raw))) {
  stop(paste0("Wastewater CSV missing required columns (expected: ",
              paste(ww_cols, collapse = ", "), ")"))
}

ww <- ww_raw %>% filter(`Pathogen Target` == "Influenza A virus")
if (nrow(ww) == 0) stop("No rows with Pathogen Target == 'Influenza A virus' in the wastewater file")

# Week End is US-formatted M/D/YYYY (not zero-padded).
ww_week <- suppressWarnings(as.Date(ww$`Week End`, format = "%m/%d/%Y"))
if (any(is.na(ww_week))) stop("The Week End could not be parsed.")

ww_val <- suppressWarnings(parse_number(ww$`National WVAL`))
if (any(is.na(ww_val))) stop("The National WVAL could not be parsed.")

ww <- data.frame(week = ww_week, wval = as.numeric(ww_val), stringsAsFactors = FALSE) %>%
  distinct(week, .keep_all = TRUE) %>%
  arrange(week)

cat("[val] wastewater series:", nrow(ww), "weeks,",
    format(min(ww$week), "%Y-%m-%d"), "to", format(max(ww$week), "%Y-%m-%d"), "\n")

# Same weekday as the flu series, and evenly spaced at 7 days.
flu_wday <- unique(weekdays(df$week))
ww_wday  <- unique(weekdays(ww$week))
if (length(ww_wday) != 1 || length(flu_wday) != 1 || ww_wday != flu_wday) {
  stop(paste0("Wastewater weekday (", paste(ww_wday, collapse = "/"),
              ") does not match flu weekday (", paste(flu_wday, collapse = "/"), ")"))
}
ww_gaps <- as.numeric(diff(ww$week))
if (any(ww_gaps != 7)) stop("Wastewater weeks are not evenly spaced at 7 days")
cat("[val] wastewater weekday matches flu (", ww_wday, ") and spacing is 7 days: OK\n")

overlap <- inner_join(df %>% select(week), ww %>% select(week), by = "week")
cat("[val] flu/wastewater overlap:", nrow(overlap), "weeks,",
    format(min(overlap$week), "%Y-%m-%d"), "to", format(max(overlap$week), "%Y-%m-%d"), "\n")
...
cat("\nSeason Start Week:", format(season_start, "%Y-%m-%d"), "\n")
...
# Unchanged from 03_forecast.R -- the reference-date set must match exactly so
...
univariate_train_start <- min(df$week)
...
# ---- Rule 1W: Lagged Regressor Construction ----------------------------------
WVAL_LAG <- 3  # weeks; equals the maximum horizon so all xreg is observed at r

cat("\n--- Rule 1W validations (lagged regressor) ---\n")
cat("[val] wastewater lag:", WVAL_LAG, "weeks\n")
cat("[val] regressor week used per horizon (all observed at reference date r):\n")
cat("        horizon 1 -> target r +  7 -> regressor week r - 14\n")
cat("        horizon 2 -> target r + 14 -> regressor week r -  7\n")
cat("        horizon 3 -> target r + 21 -> regressor week r\n")

# Lookup: wval keyed by week, so wval_at(t) is the value at calendar week t.
wval_lookup <- setNames(ww$wval, as.character(ww$week))
wval_at <- function(dates) unname(wval_lookup[as.character(dates)])

# wval_lag3(t) = wval at week t - 7 * WVAL_LAG
df$wval_lag3 <- wval_at(df$week - 7 * WVAL_LAG)

# Every reference date must have all three regressor weeks present.
missing_ref <- character(0)
for (r in reference_dates) {
  rd <- as.Date(r, origin = "1970-01-01")
  need <- rd + 7 * (1:3) - 7 * WVAL_LAG   # r-14, r-7, r
  if (any(is.na(wval_at(need)))) missing_ref <- c(missing_ref, format(rd, "%Y-%m-%d"))
}
if (length(missing_ref) > 0) {
  stop(paste0("Missing wastewater regressor weeks for reference date(s): ",
              paste(missing_ref, collapse = ", ")))
}
cat("[val] all 3 regressor weeks present for every reference date: OK\n")

# ---- Rule 2W: Training Window Restriction ------------------------------------
# Rows without a lagged regressor cannot be used to fit an ARIMAX model.
df_x <- df %>% filter(!is.na(wval_lag3)) %>% arrange(week)
if (nrow(df_x) == 0) stop("No rows have a non-NA lagged wastewater regressor")

arimax_train_start <- min(df_x$week)
weeks_dropped <- sum(df$week < arimax_train_start)

cat("\n--- Rule 2W validations (training window) ---\n")
cat("[val] univariate (03_forecast.R) training start:",
    format(univariate_train_start, "%Y-%m-%d"), "\n")
cat("[val] ARIMAX training start:", format(arimax_train_start, "%Y-%m-%d"), "\n")
cat("[val] flu history weeks dropped for ARIMAX:", weeks_dropped,
    "of", nrow(df), "(", nrow(df_x), "usable )\n")

x_gaps <- as.numeric(diff(df_x$week))
if (any(x_gaps != 7)) stop("ARIMAX training window is not contiguous after dropping NA regressor rows")
cat("[val] restricted training window contiguous (7-day spacing): OK\n")

n_first <- sum(df_x$week <= min(reference_dates))
if (n_first < 52) {
  stop(paste0("Only ", n_first, " training rows at the first reference date (need >= 52)"))
}
cat("[val] training rows at first reference date:", n_first, "(>= 52): OK\n")
...
# ---- Rules 4W & 5W: fit per reference date, forecast, reshape long -----------
cat("\n--- Rule 4W & 5W validations ---\n")
...
for (r in reference_dates) {
  ...
  # Expanding window: all usable weeks with week <= reference_date.
  train <- df_x %>% filter(week <= rd) %>% arrange(week)

  # Rule 4 validations on the training window (carried over unchanged).
  wk <- train$week
  ...
  # Rule 4W: external regressor checks.
  xreg_train <- matrix(train$wval_lag3, ncol = 1,
                       dimnames = list(NULL, "wval_lag3"))
  if (nrow(xreg_train) != length(y)) {
    stop(paste0("xreg rows != response length at ref ", format(rd, "%Y-%m-%d")))
  }
  if (any(is.na(xreg_train))) stop(paste0("xreg contains NA at ref ", format(rd, "%Y-%m-%d")))
  if (length(unique(as.numeric(xreg_train))) <= 1) {
    stop(paste0("Regressor is constant (rank-deficient) at ref ", format(rd, "%Y-%m-%d")))
  }

  # Future regressor: horizon h uses wval at (r + 7h) - 7*WVAL_LAG.
  xreg_future <- matrix(wval_at(rd + 7 * (1:3) - 7 * WVAL_LAG), ncol = 1,
                        dimnames = list(NULL, "wval_lag3"))
  if (nrow(xreg_future) != 3 || ncol(xreg_future) != 1 || any(is.na(xreg_future))) {
    stop(paste0("xreg_future malformed at ref ", format(rd, "%Y-%m-%d")))
  }
  ...
  # Coefficient on the wastewater regressor (name depends on d: with
  # differencing auto.arima keeps the supplied column name).
  cf <- coef(fit)
  beta_idx <- which(names(cf) == "wval_lag3")
  beta <- if (length(beta_idx) == 1) unname(cf[beta_idx]) else NA_real_

  cat("[val] ref", format(rd, "%Y-%m-%d"),
      "| sorted+unique+evenly-spaced OK | n =", length(y),
      "| numeric/non-neg/no-NA OK | non-constant OK",
      "| xreg OK (n =", nrow(xreg_train), ", future 3x1 OK )",
      "| ARIMA(", ord["p"], ",", ord["d"], ",", ord["q"], ")",
      "| beta_wval_lag3 =", if (is.na(beta)) "NA" else sprintf("%.2f", beta),
      "| fit OK\n")

  fc <- forecast(fit, h = 3, xreg = xreg_future, level = LEVELS)
...
# ---- Write FluSight long CSV (WVAL_ prefixed) --------------------------------
...
p <- ggplot() +
  ...
  labs(
    ...
    title = paste0("USA 1-, 2-, & 3-Week-Ahead Influenza Hospitalization Forecast, ",
                   "Wastewater-Informed ARIMAX (2025-26 Season)")
  ) +
  theme_minimal() +
  theme(
